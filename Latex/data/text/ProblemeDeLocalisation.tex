...
Dans la suite de ce mémoire, la position du robot est définie par ses coordonnées dans un espace à deux dimensions ainsi que par son orientation. Les algorithmes présentés ne se limitent pas à cette situation et peuvent être utilisés pour un espace à un nombre de dimensions supérieures. L'état des bras des robots industriels~\cite{osha.gov} peut également être représenté à l'aide de l'angle de chaque articulation du bras. Toutefois, les principes sont plus simples à comprendre dans cette situation et elle est souvent suffisante pour les robots mobiles. Formellement, la position d'un robot à l'instant $t$ dans un espace à deux dimensions peut-être définie par le vecteur :
$$ x_t = \begin{pmatrix} x \\ y \\ \theta \end{pmatrix}$$

où $x$ et $y$ correspondent aux coordonnées dans l'espace à deux dimensions et $\theta$ correspond à l'orientation du robot (voir ~\ref{PR2D}). 
...
L'étape de mise à jour permet de mettre à jour la moyenne et  la matrice de covariance à l'aide des observations et des erreurs de mesure liées à l'observation. L'importance associée à l'observation au temps $t$ est formulée à l'aide du gain de Kalman exprimé à la ligne 4 de l'algorithme. Cette importance est fonction de la certitude en la position du robot et de la certitude de l'observation. 

La matrice $R_t $ et  la matrice $Q_t$ correspondent respectivement à la covariance des fonctions gaussiennes centrées en zéro associées aux erreurs de mouvement et aux erreurs d'observation. La matrice $R_t$  est de dimensions $n \times n$ où $n$ est la taille du vecteur de position $x_t$. La matrice $Q_t$ est de dimensions $k \times n$ où $k$ est la dimension du vecteur d'observation $z_t$. Les matrices $A_t$ et $B_t$ sont respectivement de dimensions $n \times n$ et $ n \times m $ où $m$ correspond à la dimension du vecteur de contrôle. La matrice $B$ permet d'exprimer la transformation utile pour exprimer le vecteur de contrôle $u_t$ dans les mêmes variables que le vecteur de position  $\mu$. De même la matrice $C $ de dimensions $k \times n$ permet d'exprimer la transformation utile pour exprimer le vecteur de postillon dans les mêmes variables que le vecteur d'observation. 

Dans l'algorithme de Kalman, l'ensemble des opérations est effectué sur des matrices. Les matrices manipulées deviennent de grande taille lorsque la position $x_t$ et donc la moyenne $\mu_t$ deviennent de taille importante. Rappelons que $x_t$ représente la position du robot. Dans le cas d'un robot avec plusieurs articulations, la position dans l'espace de chaque articulation est contenue dans ce vecteur. Ce qui montre que la taille de ce vecteur peut augmenter rapidement. De plus, la taille de la matrice de covariance est le carré de la taille de ce vecteur de position. Il est donc important d'utiliser des librairies performantes pour la manipulation de matrice. Cela évite de rendre cet algorithme inutilisable. Finalement, certaines propriétés de ces matrices permettent de diminuer le temps de calcul. L'inversion de $(C_t \overline{\Sigma}_t C^T_t + Q_t )^{-1}$ qui est une matrice symétrique peut être réalisée à l'aide de la décomposition de Cholesky~\cite{wiki:cholesky}.

En appliquant les recommandations de manipulations de matrice, l'algorithme du filtre de Kalman a une bonne complexité. La complexité est de $O(n^{2.4}+ k^{2.4} )$ où $k$ représente la taille du vecteur d'observation $z_t$ et $n$ la taille du vecteur de position $x_t$. Dans beaucoup d'applications pratiques la taille de $n$ a tendance à être supérieure à celle de $k$. La complexité est donc dominée par $O(n^{2.4})$  
...
Pour pouvoir intégrer l'erreur de mouvement dans l'espace de position du robot il est nécessaire de l'exprimer dans les mêmes variables que la position du robot. En effet, le vecteur de position du robot est exprimé en $(x,y,\theta)$ or l'erreur de contrôle est exprimée sur les variables $(d, \gamma) $. La matrice $V_t$ correspond au développement de Taylor du premier ordre en $\mu_{t-1}$. Elle est définie comme suit : 
$$
V_t = 
\begin{pmatrix}
...
\end{pmatrix}
$$
La formule suivante permet d'exprimer l'approximation de l'erreur des commandes dans les mêmes variables que le vecteur de position du robot. 
$$
R_t= V_tM_tV_t^T
$$
...
\section{EKF SLAM}
EKF SLAM est un algorithme SLAM(voir algorithme~\ref{alg:EKFSLAM}) et comme son nom le laisse penser il est basé sur l'algorithme de localisation EKF. Il utilise également une gaussienne pour représenter son état. Cependant, l'état contient à l'instant $t$ en plus de la pose du robot, la carte qui est représentée comme l'ensemble des poses des repères découverts à l'instant $t$. Cet algorithme est aussi découpé en deux étapes importantes, l'étape de prédiction ainsi que l'état de correction. Dans l'état de prédiction, la fonction de mouvement n'influence pas les valeurs de la moyenne des poses des repères. Ce qui est normal, car les repères ne bougent pas même si le robot change de position. Dans l'étape de mise à jour, lorsqu'un repère est découvert pour la première fois il est initialisé à l'aide de la position du robot, de la distance et de l'angle auxquels il a été observé. La figure ~\ref{img:EKFSLAM} présente le parcourt d'un robot qui applique l'EKF SLAM. Les petits points représentent les repères qui sont dans l'environnement du robot. Lorsque le robot observe un point de repère, l'ellipse représentant la covariance associée à ce repère est représentée sur le dessin. Plus la position du robot est incertaine et plus la position des repères qu'il va découvrir est également incertaine. Finalement, lorsque le robot scanne un repère qu'il a déjà analysé, l'ensemble des covariances des repères déjà observés sont mises à jour.  
