...
\label{sec:ModeleMovObser}

L'objectif de la localisation d'un robot est de définir sur une carte la pose du robot à un instant donné. Cette pose est dénotée par le vecteur $x_t$. 
Dans la suite de ce mémoire, la pose du robot est définie par ses coordonnées dans un espace à deux dimensions ainsi que par son orientation. Les algorithmes présentés ne se limitent pas à cette situation et peuvent être utilisés pour un espace à un nombre de dimensions supérieures. L'état des bras des robots industriels~\cite{osha.gov} peut également être représenté à l'aide de l'angle de chaque articulation du bras. Toutefois, les principes sont plus simples à comprendre dans cette situation et elle est souvent suffisante pour les robots mobiles. Formellement, la pose d'un robot à l'instant $t$ dans un espace à deux dimensions peut être définie par le vecteur~:
$$ x_t = \begin{pmatrix} x \\ y \\ \theta \end{pmatrix}$$

où $x$ et $y$ correspondent aux coordonnées dans l'espace à deux dimensions et $\theta$ correspond à l'orientation du robot (voir~\ref{PR2D}). Remarque~: lorsque «~position~» est utilisé cela se réfère plutôt aux coordonnées $x$ et $y$. La dénomination «~pose~» est utilisée lorsque d'autres paramètres sont ajoutés à ses coordonnées. 


\begin{figure}
\begin{center}
\includegraphics[scale=0.7]{./../img/PositionRobot.png}
\caption{Pose d'un robot dans un espace à deux dimensions }
\source{Probabilistic Robotics~\cite{Thrun:2005:PR:1121596}}
\label{PR2D}
\end{center}
...
Pour déterminer la pose du robot dans le temps, il faut prendre en considération les contrôles du robot (notés $u_t$) ainsi que les observations effectuées par le robot (notées $z_t$). Les algorithmes qui seront présentés par la suite suivent l'hypothèse de Markov. C'est-à-dire que l'état $x_{t}$ ne dépend que de l'état $x_{t-1}$ ainsi que des contrôles $u_{t}$ et des observations courantes $z_{t}$. La figure \ref{HM} représente cette hypothèse de Markov où les noeuds grisés correspondent aux informations connues et les noeuds blancs aux informations inférées à l'aide des informations connues. Formellement, les contrôles peuvent être définis par le vecteur suivant :
$$u_t =  \begin{pmatrix} d_t \\ \gamma_t \end{pmatrix} $$
où $d_t$ correspond à la distance parcourue et $\gamma_t$ correspond à l'angle de rotation du robot dans l'intervalle de temps séparant deux contrôles. L'odométrie permet de déterminer les commandes $u_t$. Tandis que les valeurs de $u_t$ permettent de définir itérativement les nouvelles poses à l'aide du modèle de mouvement suivant~: 
...
\input{MovementRobot}
\end{center}
\caption{Mouvement $u_t$ du robot}
\label{MovementRobot}
\end{figure}
La figure~\ref{MovementRobot} illustre la mise à jour de la pose à l'aide de la commande $u_t$.
...
où $m_{j,x}, m_{j,y} $ correspondent aux coordonnées de l'objet scanné $j$. Ces coordonnées sont stockées dans la carte. Les variables $x_t,y_t, \theta_t$ correspondent aux coordonnées de la position et l'orientation du robot au temps $t$ de l'observation.
...
où $\alpha_1,\alpha_2,\alpha_3,\alpha_4, $ sont des paramètres à déterminer et sont spécifiques à chaque robot et $\epsilon_{b^2}$ correspond à une gaussienne de moyenne nulle et de variance $b^2$. Cette formule montre que plus la distance $d$ et l'angle $\gamma$ sont importants et plus l'erreur risque d'être grande. Ceci correspond bien à la réalité.  Les valeurs de $\alpha_2d^2$ et $\alpha_4 \gamma^2$ correspondent aux erreurs où une rotation est considérée par une translation et inversement. À l'aide de ces deux formules, le modèle de mouvement auquel on a ajouté les erreurs de mesure devient donc~: 
...
où $E$ est l'espace d'état de la pose du robot et $x$ une pose spécifique du vecteur aléatoire $X$. 
...
Le théorème de la probabilité totale est également important dans l'algorithme de Markov. Dans le cas discret, il est défini comme suit:  
 ...
 $$ 
 L'algorithme de Markov décrit donc la mise à jour de la pose $x_{t-1}$ vers la pose $x_t$. Il prend en paramètre la croyance sur la pose précédente (c'est-à-dire la fonction de densité de probabilité du vecteur aléatoire $X$ au temps $t-1$), le contrôle courant, les observations courantes ainsi que la carte dans laquelle le robot évolue. Il est constitué d'une boucle principale, qui itère sur toutes les valeurs possibles pour la pose $x_t$. Ceci permet de déterminer la densité de probabilité associée à chaque pose que pourrait prendre le robot. Pour cela, cette boucle contient deux étapes importantes. La première étape se nomme la «~prédiction~» et consiste à calculer une croyance temporaire $\overline{bel}$ de la pose du robot à l'aide de $u_t$ et de la croyance de l'étape précédente $bel(x_{t-1})$. Elle se base sur le théorème de la probabilité totale. La seconde étape correspond à la mise à jour de la croyance $bel(x_t)$ à l'aide des mesures $z_t$ et de la croyance $\overline{bel}(x_t)$ calculée dans l'étape de prédiction. Cette étape se base sur la loi  d'inférence bayésienne dans laquelle $p(z_t)$ ne dépend pas de $x_t$  et est toujours égale pour toutes valeurs de $p(x_t| z_t,x_t,m)$.  Pour cette raison le $\eta $ correspond à un normalisateur qui permet de garder une loi de probabilité et il est égal à $p(z_t)^{-1}$.
...
La figure~\ref{ILM} illustre une situation où l'algorithme de Markov est appliqué. Dans cette illustration, le robot se déplace dans un monde en une dimension. Le robot est capable de se déplacer vers la droite ou la gauche. Il peut déterminer avec une certaine probabilité s'il se trouve devant une porte ou non. Il peut aussi déterminer avec une certaine probabilité la pose dans laquelle il se trouve à l'aide des déplacements qu'il a effectués. Dans l'image (a), le robot n'a encore effectué aucun déplacement ni observation et n'a aucune information initiale sur sa pose. Il a donc une probabilité uniforme de se trouver sur n'importe quel point de la carte. Dans l'image (b), le robot observe qu'il se trouve devant une porte. Cette observation permet au robot de déduire qu'il est devant une des trois portes de la carte. La probabilité autour des portes augmente en conséquence. Dans l'image (c), le robot se déplace vers la droite. Ceci implique de déplacer la fonction de la croyance. Le déplacement du robot implique une diminution de la confiance en la croyance de sa pose due aux erreurs d'estimation du déplacement. Cette diminution est représentée par un aplatissement des différentes gaussiennes. Dans l'image (d), le robot découvre à nouveau une porte. Ceci augmente la confiance en la croyance de sa pose. Et finalement, l'image (e), démontre encore une fois que les déplacements diminuent la confiance en la croyance de la pose du robot. 
...
La prédictibilité de l'environnement est un élément important dans le choix d'appliquer ou non des algorithmes probabilistes. Dans le cas d'un environnement bien structuré, comme une chaine de montage, le degré d'imprédictibilité est bien moins important que lorsque le robot évolue en ville ou dans une maison. En effet, l'environnement d'une chaine de montage est beaucoup moins sujet à des éléments imprévus comme des personnes ou objets inconnus venant s'ajouter à son environnement de travail. Par la nature imprédictible des êtres vivants, l'imprédictibilité de l'environnement augmente fortement lorsque le robot évolue dans un environnement en présence d'êtres vivants. Les algorithmes probabilistes permettent de pallier l'imprédictibilité d'un environnement. Lorsque l'environnement est fortement imprédictible, il est souvent plus prudent d'augmenter le nombre de capteurs. Des capteurs de proximité d'un robot de chaine de montage peuvent être ajoutés pour éviter un accident si une personne rentre dans le champ de mouvement du robot. Cependant, dans le cas des robots de chaines de montage, la vitesse d'exécution du robot est très importante pour la productivité de l'entreprise. Il est donc préférable de ne pas diminuer cette vitesse avec des vérifications de sécurité et plutôt d'interdire l'accès aux alentours de la zone de travail du robot.   

Les algorithmes probabilistes souffrent de deux défauts importants. Le premier est la complexité en temps de calcul de l'algorithme qui augmente. En effet, dans les algorithmes probabilistes on considère toute la fonction de densité de probabilité lorsque les algorithmes classiques (non probabilistes) ne considèrent qu'un élément. Le deuxième est le besoin d'utiliser des approximations de la densité de probabilité exacte. Considérer la fonction de densité exacte devient vite impossible à calculer et il est donc indispensable d'utiliser des approximations comme des gaussiennes ou un nombre restreint des éléments de la fonction de densité. Dans certaines situations, ces représentations peuvent être éloignées de la réalité. Cependant, l'augmentation de la puissance de calcul des processeurs ainsi que les recherches d'algorithmes plus efficients permettent de grandes évolutions dans le domaine. Toutefois, ces deux points restent encore problématiques. Ils sont donc discutés dans la présentation des algorithmes de localisation suivants.   


\section{Algorithmes de résolution du problème de localisation}
\label{algoLoc}
L'algorithme de Markov permet de donner l'idée générale des algorithmes de localisation. Cependant, pour pouvoir implémenter concrètement un algorithme de localisation, il reste un certain nombre de questions auxquelles il faut encore répondre. La plus importante correspond à la représentation de la fonction de probabilité. Deux grandes approches existent. La première définit une loi de probabilité à l'aide de ses paramètres. Et la seconde représente la probabilité à l'aide d'un certain nombre d'éléments discrets. Les sections suivantes discutent de ces deux approches. Elles présentent les algorithmes EKF et MCL qui font partie des algorithmes de localisation les plus connus et qui présentent de bons résultats en pratique. 

\subsection{Algorithme paramétrique(EKF)}
\label{sec:Algorithme paramétrique(EKF)}
Les algorithmes paramétriques de localisation permettent de représenter la croyance de la pose d'un robot à l'aide d'une loi de probabilité. Les concepts mathématiques de l'algorithme de Kalman, qui est un algorithme paramétrique, ont été développés dans les années 60~\cite{Kalman61newresults}. L'algorithme de Kalman n'est pas uniquement utilisé dans la localisation en robotique. Il est aussi par exemple utilisé dans la restauration d'image 2D~\cite{1169273}, ou bien dans la gestion de la congestion du trafic routier~\cite{TrafficFlow}. Dans le cas de l'algorithme Kalman Filter(KF) (voir algorithme~\ref{alg:kalmanFilter}) la loi de probabilité est une loi normale. Dans le cas unidimensionnel, elle dépend donc de deux paramètres, son espérance $\mu$ et son écart type $\sigma $ ou $\sigma^2$ qui correspond à la variance. Cette loi normale est une loi normale multivariée lorsque le vecteur de pose est composé de plusieurs éléments. Dans le cas de notre robot, on définit alors la moyenne de cette fonction multivariée comme suit~: 
$$\mu = x_t =  \begin{pmatrix} x\\y\\ \theta  \end{pmatrix}$$ 
Dans ce cas, la matrice de covariance  est définie comme suit: 
$$
 \Sigma = Var(\vec{x_t})= 
\begin{pmatrix} 
...
\end{pmatrix}
$$
Cette matrice permet d'exprimer les erreurs sur la pose du robot. Les valeurs de la diagonale de la matrice augmentent durant le parcours du robot. Elles diminuent lorsque le robot effectue une observation qui lui permet d'augmenter sa certitude en sa pose. Une explication détaillée de la matrice de covariance est donnée dans la section~\ref{sectionCovariance}. On y trouve également une technique pour représenter visuellement la matrice de covariance.  

L'algorithme de Kalman  est composé des deux mêmes étapes principales que l'algorithme de Markov, la prédiction et la mise à jour. La prédiction consiste à déplacer la moyenne de la gaussienne en fonction des mouvements du robot. Elle adapte la matrice de covariance en fonction de l'erreur associée au mouvement.
L'étape de mise à jour permet de mettre à jour la moyenne et  la matrice de covariance à l'aide des observations et des erreurs de mesure liées à l'observation. L'importance associée à l'observation au temps $t$ est formulée à l'aide du gain de Kalman exprimé à la ligne 4 de l'algorithme. Cette importance est fonction de la certitude en la pose du robot et de la certitude de l'observation. 

La matrice $R_t $ et  la matrice $Q_t$ correspondent respectivement à la covariance des fonctions gaussiennes centrées en zéro associées aux erreurs de mouvement et aux erreurs d'observation. La matrice $R_t$  est de dimensions $n \times n$ où $n$ est la taille du vecteur de pose $x_t$. La matrice $Q_t$ est de dimensions $k \times n$ où $k$ est la dimension du vecteur d'observation $z_t$. Les matrices $A_t$ et $B_t$ sont respectivement de dimensions $n \times n$ et $ n \times m $ où $m$ correspond à la dimension du vecteur de contrôle. La matrice $B$ permet d'exprimer la transformation utile pour exprimer le vecteur de contrôle $u_t$ dans les mêmes variables que le vecteur de pose $\mu$. De même la matrice $C $ de dimensions $k \times n$ permet d'exprimer la transformation utile pour exprimer le vecteur de pose dans les mêmes variables que le vecteur d'observation. 

Dans l'algorithme de Kalman, l'ensemble des opérations est effectué sur des matrices. Les matrices manipulées deviennent de grande taille lorsque la pose $x_t$ et donc la moyenne $\mu_t$ deviennent de taille importante. Rappelons que $x_t$ représente la pose du robot. Dans le cas d'un robot avec plusieurs articulations, la position dans l'espace de chaque articulation est contenue dans ce vecteur. Ceci montre que la taille de ce vecteur peut augmenter rapidement. De plus, la taille de la matrice de covariance est le carré de la taille de ce vecteur de pose. Il est donc important d'utiliser des librairies performantes pour la manipulation de matrice. Cela évite de rendre cet algorithme inutilisable. Finalement, certaines propriétés de ces matrices permettent de diminuer le temps de calcul. L'inversion de $(C_t \overline{\Sigma}_t C^T_t + Q_t )^{-1}$ qui est une matrice symétrique peut être réalisée à l'aide de la décomposition de Cholesky~\cite{wiki:cholesky}.

En appliquant les recommandations de manipulations de matrice, l'algorithme du filtre de Kalman a une bonne complexité. La complexité est de $O(n^{2.4}+ k^{2.4} )$ où $k$ représente la taille du vecteur d'observation $z_t$ et $n$ la taille du vecteur de pose $x_t$. Dans beaucoup d'applications pratiques la taille de $n$ a tendance à être supérieure à celle de $k$. La complexité est donc dominée par $O(n^{2.4})$  
...
quelques années plus tard  par la NASA~\cite{Smith1962} pour faire face au fait que la plupart des systèmes physiques ne sont pas linéaires. 
Pour ce faire les fonctions $g$ et $h$ ont été introduites dans l'algorithme EKF. Ces fonctions sont des fonctions linéaires ou non linéaires. Elles correspondent au modèle de mouvement et au modèle d'observation du robot. Les matrices $G$ et $H$ correspondent à la décomposition de Taylor du premier ordre des fonctions $g$ et $h$. Cette décomposition de Taylor permet de rendre linéaires les fonctions $g$ et $h$. EKF applique ces fonctions linéaires sur le calcul de la matrice de variance. Il est donc maintenant possible de passer en paramètres un modèle de mouvement non linéaire et un modèle d'observation non linéaire en gardant une représentation gaussienne de la pose du robot.  Contrairement au filtre de Kalman classique où les modèles sont obligatoirement linéaires pour préserver des fonctions de répartition gaussienne. 

Cette décomposition de Taylor n'ajoute pas de complexité à l'algorithme. En effet, elle est faite analytiquement au préalable. Cependant, la linéarisation implique que l'algorithme EKF retourne une approximation de la fonction de densité de probabilité associée au modèle de mouvement et d'observation non linéaires.  La figure~\ref{img:EKFlinearisation} représente la linéarisation de la fonction $g$. On voit clairement que si la fonction $g$ est appliquée sans linéarisation sur l'ensemble de la croyance,  la croyance n'est plus une gaussienne. On peut également voir que la gaussienne construite à l'aide de la linéarisation correspond à une approximation de la vraie gaussienne. Dans le cas d'un modèle de mouvement linéaire et un modèle d'observation linéaire, l'algorithme EKF fournit les mêmes résultats que l'algorithme KF. 

\begin{figure}
\begin{center}
\includegraphics{./../img/ekf_linearisation.png}
\caption{EKF linéarisation }
\source{Probabilistic Robotics~\cite{Thrun:2005:PR:1121596}}
\label{img:EKFlinearisation}
\end{center}
\end{figure}


L'algorithme~\ref{alg:EKFLocalisation} décrit l'algorithme EKF appliqué à la localisation de notre robot. Le robot utilise une carte composée de repères (comme défini dans la section~\ref{section:carteRepères}) pour se localiser. Cette carte est passée en paramètre en plus des paramètres classiques de l'algorithme EKF. Cette version de l'EKF localisation permet d'intégrer plusieurs observations au temps $t$ grâce à la boucle qui itère sur chaque observation. Le modèle de mouvement et le modèle d'observation correspondent à ceux définis dans la section~\ref{sec:ModeleMovObser}. La fonction $g$, qui correspond au modèle de mouvement, est définie comme suit~: 
$$ g =
\begin{pmatrix}
x_t \\
y_t\\
...
\end{pmatrix}
$$ 
Le développement de Taylor du premier ordre de la fonction $g$ en $\mu_{t-1}$ est donné par la matrice $G$ suivante~: 

$$
G =
\begin{pmatrix}
\frac{ \partial x_t}{ \partial \mu_{t-1,x} } & \frac{ \partial x_t}{ \partial \mu_{t-1,y} }& \frac{ \partial x_t}{ \partial \mu_{t-1, \theta} } \\
 \frac{ \partial y_t}{ \partial \mu_{t-1,x} } &  \frac{ \partial y_t}{ \partial \mu_{t-1,y} }&  \frac{ \partial y_t}{ \partial \mu_{t-1,\theta} } \\
 \frac{ \partial \theta_t}{ \partial \mu_{t-1,x} } &\frac{ \partial \theta_t}{ \partial \mu_{t-1,y} }  & \frac{ \partial \theta_t}{ \partial \mu_{t-1,\theta} } 
\end{pmatrix}=
\begin{pmatrix}
...
\end{pmatrix}
$$
La fonction $h$ qui correspond au modèle d'observation est définie comme suit~: 
$$
h = \begin{pmatrix}
\sqrt{ (m_{j,x}-\overline{\mu}_{t,x} )^2 + (m_{j,y}-\overline{\mu}_{t,y})^2}\\
atan2(m_{j,y}-\overline{\mu}_{t,y},m_{j,x}-\overline{\mu}_{t,x} )- \overline{\mu_{t,\theta}}\\
\end{pmatrix}
$$
où $m_{j,x},m_{j,x}$ correspondent aux coordonnées du repère enregistrées dans la carte. Le développement de Taylor du premier ordre de la fonction $h$ en $\bar{\mu}_{t}$ est donné par la matrice $H$ qui est définie comme suit~: 
$$
H = 
\begin{pmatrix}
\frac{\partial d^i_t}{\partial \bar{\mu}_{t,x}} & \frac{\partial d^i_t}{\partial \bar{\mu}_{t,y}} & \frac{\partial d^i_t}{\partial \bar{\mu}_{t,\theta}} \\
\frac{\partial \rho^i_t}{\partial \bar{\mu}_{t,x}} & \frac{\partial \rho^i_t}{\partial \bar{\mu}_{t,y}} & \frac{\partial \rho^i_t}{\partial \bar{\mu}_{t,\theta}}
\end{pmatrix}
=
 \begin{pmatrix}
...
Pour pouvoir intégrer l'erreur de mouvement dans l'espace de pose du robot, il est nécessaire de l'exprimer dans les mêmes variables que la pose du robot. En effet, le vecteur de pose du robot est exprimé en $(x,y,\theta)$ or l'erreur de contrôle est exprimée sur les variables $(d, \gamma) $. La matrice $V_t$ correspond au développement de Taylor du premier ordre en $\mu_{t-1}$. Elle est définie comme suit : 
$$
V_t = 
\begin{pmatrix}
\frac{\partial x_t}{\partial d_t } && \frac{\partial x_t}{\partial \gamma_t }  \\
\frac{\partial y_t }{\partial d_t } && \frac{\partial y_t }{\partial \gamma_t } \\
\frac{\partial \theta_t}{\partial d_t } && \frac{\partial \theta_t}{\partial \gamma_t } 
\end{pmatrix}
=
\begin{pmatrix}
...
\end{pmatrix}
$$
La formule suivante permet d'exprimer l'approximation de l'erreur des commandes dans les mêmes variables que le vecteur de pose du robot. 
$$
R_t= V_tM_tV_t^T
$$
...
\State $q \gets (m_{j,x}-\overline{\mu}_{t,x} )^2 + (m_{j,y}-\overline{\mu}_{t,y})^2$ \Comment{$m_{j,x} m_{j,y}$  coordonnées du repère }
\State $ \hat{z}^i_t \gets 
\begin{pmatrix}
\sqrt{q}\\
atan2(m_{j,y}-\overline{\mu}_{t,y},m_{j,x}-\overline{\mu}_{t,x} )- \overline{\mu}_{t,\theta}\\
\end{pmatrix} 
$
\State $H^i_t \gets
\begin{pmatrix}
...
À l'inverse des algorithmes paramétriques, les algorithmes non paramétriques ne sont pas basés sur une loi de probabilité connue dont l'algorithme arrange les paramètres pour correspondre au mieux à la croyance de la pose. Dans les algorithmes non paramétriques, la croyance est représentée par un nombre déterminé de poses supposées. Une probabilité est associée à ces poses supposées. Plusieurs techniques existent pour les représenter. 

La première technique consiste à découper la carte de l'environnement en une grille où chaque élément de la grille correspond aux coordonnées supposées du robot~\cite{4621305}. Pour ajouter l'orientation du robot dans cette représentation sous forme de grille, il faut multiplier le nombre de cases par le nombre d'angles d'orientation que peut prendre le robot. Dans la représentation~\ref{img:gridMap}, seules trois orientations sont possibles. Ces trois orientations correspondent aux trois plans de la représentation. Comme on peut s'en rendre compte, il est très important de définir la bonne granularité de la découpe. Une découpe trop importante augmente le temps de calcul, alors qu'une grille trop peu découpée rend la localisation trop peu précise. Dans ce type de découpe, plus la carte est grande et plus le temps de calcul est important. 

Dans le cas de l'algorithme de Monte Carlo Localization (MCL), aussi appelé particle filter localization~\cite{bib:Rekleitis2004}  car il utilise un filtre à particules, une approche différente a été choisie. Dans cet algorithme (voir l'algorithme~\ref{alg:MCL }) la croyance de la pose est représentée par $M$ particules (voir la figure~\ref{img:MCL}). Chaque particule est considérée comme une hypothèse sur la pose du robot. Les traits gris correspondent aux particules et le niveau de rouge représente la probabilité associée. Plus une région de la carte contient de particules et plus la précision de cette zone de la croyance est importante. Plus le poids associé à la particule est important et plus la probabilité que le robot s'y trouve est grande. L'algorithme est composé de l'étape de prédiction et de l'étape de mise à jour comme l'algorithme de Markov. Pour chaque particule, ces deux étapes sont appliquées. L'étape de prédiction applique le mouvement à chaque particule. L'étape de mise à jour détermine une valeur en fonction de la probabilité d'avoir une telle observation en étant à la pose de la particule. Une fois ces deux étapes appliquées à chaque particule, la partie finale consiste à redéfinir l'ensemble des particules. Le nouvel ensemble est construit avec les particules qui ont la plus grande probabilité associée à l'étape précédente. Contrairement aux algorithmes basés sur la découpe de la carte en une grille, MCL n'implique pas un temps de calcul supplémentaire lorsque la taille de la carte augmente. Cependant, il est possible de choisir d'augmenter le nombre de particules pour augmenter la précision. 

MCL souffre d'un problème important, en particulier quand $M< 50$ et que l'environnement du robot est grand. Il peut arriver que l'ensemble des particules converge vers une pose erronée. Une fois cette convergence atteinte, il est difficile d'en sortir. Pour pallier à ce problème, à chaque itération, un certain nombre de particules sont redistribuées aléatoirement dans la carte. 
...
\For {$ m = 1$ to  M }
\State  $ x^{[m]}_t \gets sample\_motion\_model(u_t , x^{[m]}_{t-1})$ \Comment{prédiction}
\State $w_t^{[m]} \gets measurement\_model(z_t , x_t^{[m]},m)$ \Comment{mise à jour}
\State $ \overline{\mathcal{X}}_t \gets \overline{\mathcal{X}}_t + \langle  x^{[m]}_t , w_t^{[m]} \rangle $
\EndFor

\For {$ m = 1$ to  M }
\State draw i  with probability proportional to $w^{[i]}_t$
...
Le tableau~\ref{MCLVSEKF} donne et compare les principales caractéristiques de l'algorithme EKF et MCL. Ce tableau comparatif permet de mettre en valeur qu'aucun algorithme n'est globalement meilleur. Ils possèdent chacun leurs qualités et défauts. Les défauts de l'un se révèlent être les qualités de l'autre. Par exemple, EKF est efficient en temps et mémoire contrairement au MCL dont l'efficience dépend fortement du nombre de particules. 

Cependant, MCL est plus robuste qu'EKF. En effet, la représentation de la fonction de probabilité à l'aide d'une loi normale permet à EKF d'être efficient. Cependant, le revers de cette efficience est qu'il est moins robuste lorsque la fonction de probabilité est fortement différente d'une loi normale. Prenons l'exemple d'un long couloir avec un grand nombre de portes et où le robot n'est pas capable de distinguer les portes. Dans cette situation, MCL peut donner de meilleures performances. En effet, EKF assume que la croyance de la pose est proche d'une distribution gaussienne et a donc de mauvaises performances lorsque la croyance correspond plutôt à une distribution multimodale. Pour pallier à ce problème, l'algorithme classique EKF a été amélioré. Le filtre Multi-hypothesis Tracking (MHT)~\cite{1263228} permet de représenter la croyance à l'aide d'un mixte de plusieurs gaussiennes et donc d'avoir un algorithme plus robuste et efficient. Le même type de problème se présente pour la représentation de l'erreur de mesure. Dans le cas de MCL, cette erreur de mesure ne doit pas obligatoirement suivre une gaussienne contrairement à EKF. MCL permet de représenter l'erreur de mesure par une fonction quelconque, ce qui permet de s'approcher plus facilement de la réalité physique.  

La localisation globale correspond à un problème de localisation où la pose initiale n'est pas connue. L'incertitude est donc grande à cet instant. Il s'avère qu'EKF est plus approprié pour suivre la pose d'un robot dont on connait déjà la pose initiale. En effet, une représentation unimodale est généralement une bonne représentation dans un problème où il s'agit de suivre une pose, mais pas dans un problème de localisation globale. La linéarisation dans EKF ne fait qu'accroitre ce problème en risquant de converger vers une mauvaise pose.


De plus, MCL permet de traiter directement dans l'algorithme des mesures brutes or EKF nécessite des repères. Déterminer un repère nécessite un prétraitement des données brutes. Il est donc possible à l'aide de MCL d'utiliser directement les valeurs de capteurs de distance entre le robot et des murs pour les comparer avec une carte représentant les murs. Ceci n'est pas possible à l'aide d'EKF qui nécessite une carte composée d'un nombre limité de repères qui permet de localiser le robot à l'aide des repères mesurés dans son environnement. Finalement, en pratique il s'avère que MCL est plus simple à implémenter qu'EKF.
...
\hline 
\end{tabular}
\caption{Comparaison d'EKF et de MCL}
\label{MCLVSEKF} 
\end{center}
\end{table}
...
Les algorithmes de type SLAM (Simultaneous Localization And Mapping) sont l'étape suivante vers l'indépendance des robots. En effet, dans les algorithmes de simple localisation, la carte de l'environnement doit être construite avant de la passer en paramètre aux algorithmes de localisation. Comme son nom l'indique, dans un algorithme SLAM la carte est construite en parallèle avec la localisation du robot. La section suivante présente l'EKF SLAM qui est l'extension de l'EKF localisation. Le FastSLAM est un autre algorithme SLAM connu qui est l'extension du MCL. Les particules dans le FastSLAM sont composées d'une estimation du chemin du robot et de l'ensemble des estimations des points de repère de la carte avec leur covariance associée. La description détaillée de cet algorithme est disponible dans le livre «~Probabilistic robotics~»~\cite{Thrun:2005:PR:1121596}.

\section{EKF SLAM}
EKF SLAM est un algorithme SLAM (voir algorithme~\ref{alg:EKFSLAM}) et comme son nom le laisse penser il est basé sur l'algorithme de localisation EKF. Il utilise également une gaussienne pour représenter son état. Cependant, l'état contient à l'instant $t$, en plus de la pose du robot, la carte qui est représentée comme l'ensemble des poses des repères découverts à l'instant $t$. Cet algorithme est aussi découpé en deux étapes importantes, l'étape de prédiction ainsi que l'état de correction. Dans l'état de prédiction, la fonction de mouvement n'influence pas les valeurs de la moyenne des poses des repères. Ceci est normal, car les repères ne bougent pas même si le robot change de pose. Dans l'étape de mise à jour, lorsqu'un repère est découvert pour la première fois il est initialisé à l'aide de la pose du robot, de la distance et de l'angle auxquels il a été observé. La figure~\ref{img:EKFSLAM} présente le parcourt d'un robot qui applique l'EKF SLAM. Les petits points représentent les repères qui sont dans l'environnement du robot. Lorsque le robot observe un point de repère, l'ellipse représentant la covariance associée à ce repère est représentée sur le dessin. Plus la pose du robot est incertaine et plus la position des repères qu'il va découvrir est également incertaine. Finalement, lorsque le robot scanne un repère qu'il a déjà analysé, l'ensemble des covariances des repères déjà observés sont mises à jour.  



\begin{figure}
\begin{center}
\includegraphics[scale=0.7]{./../img/ekf_slam.png}
\caption{EKF SLAM}
\source{Probabilistic Robotics~\cite{Thrun:2005:PR:1121596}}
\label{img:EKFSLAM}
\end{center}
...
\section{Graphe}
Pour pouvoir déterminer un chemin qui ne risque pas de percuter les objets de l'environnement, ces algorithmes nécessitent de posséder une ou plusieurs cartes de l'environnement du robot. Ces cartes peuvent avoir été générées dynamiquement par le robot ou bien construites au préalable. Elles serviront à produire un graphe où les noeuds représentent des positions possibles du robot et les arêtes représentent les chemins qui permettent de rejoindre ces différentes positions. La figure~\ref{CHValide} présente le graphe construit à l'aide de la carte de la figure~\ref{CHnonValide}. Pour construire ce graphe à partir de cette carte, une technique est de découper l'environnement du robot en une carte grillagée où chaque élément de la grille prend une valeur occupée ou libre selon qu'un objet se trouve à l'intérieur ou non. Les arêtes du graphe représentent le lien entre deux cases adjacentes libres dans la grille. Un cout d'une unité est associé à ces arêtes. Le cout correspond à la distance entre chaque noeud. Si une case n'est pas libre, aucun lien ne la relie. Ceci signifie qu'il ne faut pas passer par cette case pour définir le chemin du robot. Il est ainsi possible de passer de case en case pour déterminer le chemin entre un point de départ et un point d'arrivée. Le cout total du chemin correspond à la somme des couts des arêtes empruntées pour aller du point de départ jusqu'au point d'arrivée. Il est également possible de déterminer si un point d'arrivée n'est pas joignable. Ceci se produit lorsqu'il est impossible d'y accéder sans percuter des objets de l'environnement.  
...
Plus la grille a un découpage important, et plus le chemin qui est retourné est précis. Cependant, un découpage plus important augmente le temps de calcul de façon exponentielle. En plus d'un paramètre qui permet de déterminer la granularité de la carte, un paramètre qui permet de définir la distance à laquelle le robot peut être proche des murs doit être défini. Cette valeur permet de prendre en considération la largeur du robot ainsi que ses capacités de rotation. En effet, il est commun que la position du robot définisse le centre du robot, cependant celui-ci possède une largeur qu'il faut prendre en considération pour éviter de percuter les murs avec les côtés du robot. Ce paramètre permet donc de construire les chemins avec des positions qui sont toujours à une distance minimale des murs. 
...
\State Initialiser tous les sommets comme non marqués et donner un valeur $+ \infty$ à tous les labels L 
...
\section{$A^*$}
L'algorithme $A^*$ a été publié en 1968~\cite{AICPub834}  et fonctionne de façon relativement semblable à l'algorithme de Dijkstra, mais celui-ci permet d'obtenir de meilleures performances que Dijksta grâce à l'utilisation d'une heuristique. Cette heuristique donne une estimation du cout jusqu'aux destinations recherchées. Pour l'appliquer son heuristique doit être admissible. C'est-à-dire que la fonction heuristique ne doit jamais surestimer la valeur réelle du cout. Dans la recherche du plus court chemin valide de notre robot, l'heuristique représente la distance à vol d'oiseau qui est toujours plus courte ou égale à n'importe quel chemin. L'algorithme~\ref{alg:AStar} correspond au pseudocode de l'algorithme $A^*$. L'algorithme $A^*$, comme celui de Dijkstra, est découpé en deux parties principales. La première partie consiste à déterminer les plus courts chemins entre un sommet initial et les autres sommets. Elle est suivie de la reconstruction du chemin entre le sommet d'arrivée et le sommet de départ. 

Intuitivement, l'algorithme $A^*$, composé de l'heuristique estimant la distance à vol d'oiseau, explore itérativement le noeud joignable et étant estimé par l'heuristique comme le plus proche du noeud cible. Lorsque ce noeud est choisi, ses voisins deviennent joignables. Ensuite, l'heuristique estime la distance entre ses voisins et le noeud cible. Et ce, itérativement jusqu'au noeud cible. Dans un labyrinthe, cet algorithme peut avoir des performances moins bonnes. La figure~\ref{AstarLab} illustre cette situation. Il est clair qu'à vol d'oiseau le noeud à gauche du départ est le chemin unique et le plus court pour atteindre l'arrivée. Cependant l'algorithme $A^*$ va commencer par explorer les noeuds à droite du départ pour arriver jusqu'au cul-de-sac et finalement explorer le noeud à gauche du départ. Cependant, l'algorithme finit toujours par retourner le chemin le plus court.     
...
\begin{algorithmic}[1]
\Procedure{$A^*$ }{$G = (S,A), S_{start} ,S_{goal}$}  
